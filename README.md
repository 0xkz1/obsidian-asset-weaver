...
AssetWeaverは、ObsidianのVault内にある画像アセットを自動的にスキャンし、ローカルのVLM（ビジョン言語モデル）を活用して構造化されたMarkdownサイドカーファイルを生成するプラグインです。クリエイティブな専門家やアーティスト、研究者が、クラウドAPIに頼ることなく、視覚的なライブラリをナレッジグラフへ統合するのに最適です。
