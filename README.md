...
flowchart TD
    ...
    GetFiles --> Skip
    Skip --> FilterImages
    FilterImages -- "No" --> Finished
    FilterImages -- "Yes" --> LoopStart
    
    LoopStart --> ReadBinary
    ReadBinary --> CheckEmpty
    CheckEmpty -- "Yes (Corrupted)" --> ErrorNotice
    CheckEmpty -- "No (Valid)" --> Resize
    
    Resize --> ScanBacklinks
    ScanBacklinks --> CallVLM
    CallVLM --> CleanJSON
    CleanJSON --> FormatMD
    FormatMD --> CreateFile
